  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="container-section">
        {/* Main frame containing both text and comparison table */}
            ...
            <table className="w-full table-auto">
              ...
              <tbody>
                {comparisonData.map((row, index) => (
                  <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} ${row.highlight ? 'bg-blue-50' : ''}`}>
                    ...
                    <td className="py-4 px-6 text-center border-t border-gray-200 font-medium text-biznex-navy">
                      <div className="flex items-center justify-center gap-2">
                        {row.biznex}
                        {row.highlight && <CheckCircle className="h-4 w-4 text-biznex-secondary" />}
                      </div>
                    </td>
